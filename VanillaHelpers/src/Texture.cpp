...
static void PatchTextureSizeLimit() {
    ...
    Common::PatchBytes(reinterpret_cast<void*>(Offsets::PATCH_TEXTURE_SIZE_1 + 1), &ja, sizeof(ja));
    Common::PatchBytes(reinterpret_cast<void*>(Offsets::PATCH_TEXTURE_SIZE_2 + 1), &ja, sizeof(ja));

    const uint32_t maxSize = 0x400; // 1024
    Common::PatchBytes(reinterpret_cast<void*>(Offsets::PATCH_MIPBITS_ALLOC_PUSH + 1), &maxSize, sizeof(maxSize));
    Common::PatchBytes(reinterpret_cast<void*>(Offsets::PATCH_MIPBITS_ALLOC_MOV_EDX + 1), &maxSize, sizeof(maxSize));
}
...
static void InstallNewAsyncFileBuffer() {
    ...
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_ASYNC_BUFFER_ADDR_TEX_LOAD_IMAGE + 2),
        &newBase,
        sizeof(newBase)
    ); // TextureLoadImage
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_ASYNC_BUFFER_ADDR_BLP_PATH + 2), &newBase, sizeof(newBase)
    ); // IModelCreate/BLP path
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_ASYNC_BUFFER_ADDR_PUMP + 2), &newBase, sizeof(newBase)
    ); // Pump/refill

    // Patch buffer max size (MOV r32, imm32)
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_ASYNC_BUFFER_SIZE_TEX_LOAD_IMAGE + 1),
        &newSize,
        sizeof(newSize)
    ); // TextureLoadImage
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_ASYNC_BUFFER_SIZE_BLP_PATH + 1), &newSize, sizeof(newSize)
    ); // IModelCreate/BLP path
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_ASYNC_BUFFER_SIZE_PUMP + 1), &newSize, sizeof(newSize)
    ); // Pump/refill

    // Reset usage counter so the very first scheduling starts at newBase+0
    const uint32_t zero = 0;
    Common::PatchBytes(reinterpret_cast<void*>(Offsets::VAR_ASYNC_BUFFER_USAGE_COUNTER), &zero, sizeof(zero));
}

static auto InitializeNewTexturePool() -> bool {
    ...
    g_bucketBlock = static_cast<uint8_t*>(VirtualAlloc(nullptr, bucketBytes, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE));
    if (g_bucketBlock == nullptr) {
        return false;
    }
...
static void PatchNewTexturePool() {
    ...
    // Pool initializer
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_INIT_POOL_HEAD + 1), &g_headBase, sizeof(g_headBase)
    ); // MOV EAX, imm32
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_INIT_POOL_COUNT + 1),
        &count,
        sizeof(count)
    ); // MOV ECX, imm32
    ...
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_CLEANUP_START_EDI + 1), &headEnd, sizeof(headEnd)
    ); // MOV EDI, imm32
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_CLEANUP_COUNT + 3),
        &count,
        sizeof(count)
    ); // MOV [local_8], imm32
    ...
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_ALLOC_PREV + 3),
        &g_prevBase,
        sizeof(g_prevBase)
    ); // 8B 1C 8D imm32
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_ALLOC_HEAD + 3),
        &g_headBase,
        sizeof(g_headBase)
    ); // 8D 04 8D imm32
    ...
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_FREE_PARAM_1 + 3), &g_baramBase, sizeof(g_baramBase)
    ); // 8B 34 95 imm32
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_FREE_PARAM_2 + 3), &g_baramBase, sizeof(g_baramBase)
    ); // 8D 1C 95 imm32

    // Iterators (0x448EC0): start@prev, outer=side, inner=side
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_ITERA_PREV_LOAD + 1), &g_prevBase, sizeof(g_prevBase)
    ); // MOV EAX, imm32
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_ITERA_OUTER_CNT + 3), &side32, sizeof(side32)
    ); // MOV [local_10], imm32
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_ITERA_INNER_CNT + 3), &side32, sizeof(side32)
    ); // MOV [local_c], imm32

    // Iterators (0x449010): start@prev, outer=side, inner cmp=side, row step = side*stride
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_ITERB_PREV_LOAD + 1), &g_prevBase, sizeof(g_prevBase)
    ); // MOV EDI, imm32
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_ITERB_OUTER_CNT + 3), &side32, sizeof(side32)
    ); // MOV [local_10], imm32
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_ITERB_INNER_CMP + 2),
        &side8,
        sizeof(side8)
    ); // 83 F8 imm8
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_ITERB_ROW_STEP + 2), &rowStep8, sizeof(rowStep8)
    ); // 83 C7 imm8

    // Stats/diagnostics (0x44B4F0): prev/param bases; outer=side; total range = count*stride
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_STAT_PREV_BASE + 2), &g_prevBase, sizeof(g_prevBase)
    ); // 8B 9E imm32
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_STAT_PARAM_BASE + 2), &g_baramBase, sizeof(g_baramBase)
    ); // 8B 86 imm32
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_STAT_OUTER_CNT + 3),
        &side32,
        sizeof(side32)
    ); // MOV [local_10+4], imm32
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_TEXPOOL_STAT_TOTAL_END + 2), &totalBytes, sizeof(totalBytes)
    ); // 81 FE imm32
}
...
static void PatchBiggerCharacterSkin(uint8_t shift) {
    ...
    // Atlas coordinates
    DWORD oldProt = 0;
    auto* coordinate = reinterpret_cast<uint32_t*>(Offsets::CONST_CHARACTER_SKIN_ATLAS_COORDINATES);
    ...
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_CHAR_ATLAS_GATE_DIM_CMP + 1), &newSize, sizeof(newSize)
    ); // CMP EAX, imm32
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_CHAR_ATLAS_CREATE_SIZE_PUSH + 1), &newSize, sizeof(newSize)
    ); // PUSH imm32 (width)
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_CHAR_ATLAS_CREATE_SIZE_MOV_EDX + 1), &newSize, sizeof(newSize)
    ); // MOV EDX, imm32 (height)

    // CPU compositor stride (three blitters: copy/masked/blend)
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_CHAR_COMPOS_COPY_STRIDE + 3), &newStride, sizeof(newStride)
    ); // MOV dword ptr [local_c], imm32

    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_CHAR_COMPOS_MASKED_STRIDE + 3), &newStride, sizeof(newStride)
    ); // MOV dword ptr [local_c], imm32

    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_CHAR_COMPOS_BLEND_STRIDE + 3), &newStride, sizeof(newStride)
    ); // MOV dword ptr [local_c], imm32

    // Clear (not sure if needed)
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_CHAR_ATLAS_CLEAR_COLOR_R + 1), &fullClear, sizeof(fullClear)
    );
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_CHAR_ATLAS_CLEAR_COLOR_G + 1), &fullClear, sizeof(fullClear)
    );

    // Character Component init (not sure if needed)
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_CHAR_ATLAS_TABLE_SIDE_PUSH + 1), &newSize, sizeof(newSize)
    );
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_CHAR_ATLAS_TABLE_SIDE_MOV_EDX + 1), &newSize, sizeof(newSize)
    );

    // Engine level clamps (not sure if needed)
    Common::PatchBytes(reinterpret_cast<void*>(Offsets::PATCH_TEX_DIM_CLAMP_W + 2), &newSize, sizeof(newSize));
    Common::PatchBytes(reinterpret_cast<void*>(Offsets::PATCH_TEX_DIM_CLAMP_H + 2), &newSize, sizeof(newSize));

    // Cleanup constants (not sure if needed)
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_CHAR_ATLAS_FREE_SIDE_PUSH_W + 1), &newSize, sizeof(newSize)
    );
    Common::PatchBytes(
        reinterpret_cast<void*>(Offsets::PATCH_CHAR_ATLAS_FREE_SIDE_PUSH_H + 1), &newSize, sizeof(newSize)
    );
}

static auto GetCharacterSkinSizeSetting() -> uint8_t {
    uint8_t size = 0;
    Game::SFile* file = nullptr;

    if ((Game::SFile_Open(R"(VanillaHelpers\ResizeCharacterSkin.txt)", &file) == 0U) || file == nullptr) {
        return size;
    }

    if (Game::SFile_Read(file, &size, sizeof(size), nullptr, nullptr, nullptr) != 0U) {
        if (isdigit(size) != 0) {
            size = size - '0';
        }
    }
...
static inline auto
TextureBlitFits(const Game::Point* offset, const Game::Rectangle* copy, const Game::Rectangle* extent) -> bool {
...
static void __stdcall TextureBlitCopy_h(
    void* unk0,
    void* unk1,
    Game::Point* srcOffset,
    Game::Point* dstOffset,
    Game::Rectangle* copy,
...
static void __stdcall TextureBlitMasked_h(
    void* unk0,
    void* unk1,
    Game::Point* dstOffset,
    Game::Point* srcOffset,
...
static void __stdcall TextureBlitBlend_h(
    void* unk0,
    void* unk1,
    Game::Point* dstOffset,
    Game::Point* srcOffset,
...
void InstallCharacterSkin() {
    //Fix - add a static guard so PatchBiggerCharacterSkin runs exactly once per process lifetime
    ...
    uint8_t const size = GetCharacterSkinSizeSetting();
    switch (size) {
        case 2:
