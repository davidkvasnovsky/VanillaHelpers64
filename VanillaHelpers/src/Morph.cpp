...
static std::unordered_map<uint64_t, std::unordered_map<uint32_t, uint32_t>> g_original; // guid -> field -> original
...
static inline auto GetFieldSlot(Game::CGPlayer_C* unit, uint32_t fieldIndex) -> uint32_t* {
    switch (fieldIndex) {
        ...
        case Game::PLAYER_VISIBLE_ITEM_19_0:
            return &unit->m_data->m_visibleItems[(fieldIndex - Game::PLAYER_VISIBLE_ITEM_1_0) / 0xC].data[0];
        default:
            return nullptr;
    }
}
...
template <typename T>
static inline void
EraseNested(std::unordered_map<uint64_t, std::unordered_map<uint32_t, T>>& m, uint64_t k1, uint32_t k2) {
    auto it = m.find(k1);
    if (it == m.end()) {
        return;
    }
...
static inline auto GetOverrideValue(uint64_t guid, uint32_t fieldIndex, morphValue_t& out) -> bool {
...
static inline auto GetRemapValue(uint32_t fieldIndex, uint32_t base, morphValue_t& out) -> bool {
...
static inline auto UsingNativeDisplay(Game::CGPlayer_C* unit) -> bool {
...
static inline auto
EvaluateWriteReplacement(Game::CGPlayer_C* unit, uint32_t fieldIndex, uint32_t incoming, uint32_t& outValue) -> bool {
    const uint64_t guid = unit->m_data->m_unitData.m_objectData.m_guid;
    morphValue_t value;
...
static void OnDisplayIDChanged(Game::CGPlayer_C* unit, uint32_t newValue) {
    ...
    if (itOriginal != g_original.end()) {
        ...
        if (itOriginalFields != itOriginal->second.end()) {
            ...
            if (race > 0) {
                ...
                g_overrides[unit->m_data->m_unitData.m_objectData.m_guid][Game::UNIT_FIELD_BYTES_0] = bytes0;
                g_overrides[unit->m_data->m_unitData.m_objectData.m_guid][Game::UNIT_FIELD_NATIVEDISPLAYID] = newValue;
    ...
    morphValue_t value;
    if (GetOverrideValue(unit->m_data->m_unitData.m_objectData.m_guid, Game::UNIT_FIELD_BYTES_0, value)) {
        EraseNested(g_overrides, unit->m_data->m_unitData.m_objectData.m_guid, Game::UNIT_FIELD_BYTES_0);
        EraseNested(g_overrides, unit->m_data->m_unitData.m_objectData.m_guid, Game::UNIT_FIELD_NATIVEDISPLAYID);
        ApplyField(unit, Game::UNIT_FIELD_BYTES_0);
        ApplyField(unit, Game::UNIT_FIELD_NATIVEDISPLAYID);
    }
}
...
static void ReloadUnitDisplay(Game::CGPlayer_C* unit) {
    uint32_t const original = unit->m_data->m_unitData.m_displayId;
    unit->m_data->m_unitData.m_displayId = 15'435;
...
static void ApplyField(Game::CGPlayer_C* unit, uint32_t fieldIndex) {
    ...
    morphValue_t value;
    bool const haveOverride = GetOverrideValue(guid, fieldIndex, value);
    ...
    if ((haveOverride || haveRemap) && EvaluateMorphValue(unit, fieldIndex, value, evaluatedValue)) {
        if (!g_original[guid].contains(fieldIndex)) {
    ...
    if (const auto itOriginal = g_original.find(guid); itOriginal != g_original.end()) {
        const auto itOriginalFields = itOriginal->second.find(fieldIndex);
        if (itOriginalFields != itOriginal->second.end()) {
            uint32_t const original = itOriginalFields->second;
            EraseNested(g_original, guid, fieldIndex);
            if (current != original) {
...
static int __fastcall CGObject_C_SetBlock_h(
    Game::CGObject_C* thisptr, void* /*edx*/, uint32_t fieldIndex, uint32_t value
) {
    if (thisptr->m_objectType != Game::OBJECT_TYPE::PLAYER || (g_overrides.empty() && g_remap.empty())) {
...
static auto ParseFactionValuesTable(void* L, int index) -> std::shared_ptr<factionValuesMap_t> {
    ...
    while (Game::Lua::Next(L, index) != 0) {
        if (Game::Lua::IsNumber(L, -2) && Game::Lua::IsTable(L, -1)) {
            ...
            Game::Lua::PushNil(L);
            while (Game::Lua::Next(L, -2) != 0) {
                if (Game::Lua::IsNumber(L, -1)) {
                    replacementList.push_back(static_cast<uint32_t>(Game::Lua::ToNumber(L, -1)));
                }
                Game::Lua::Pop(L, 1);
            }
            factionValues->emplace(static_cast<uint32_t>(Game::Lua::ToNumber(L, -2)), replacementList);
        }
        Game::Lua::Pop(L, 1);
    }
...
static int __fastcall SetUnitFieldImpl(void* L, uint32_t fieldId, const char* usage) {
    ...
    auto* player = reinterpret_cast<Game::CGUnit_C*>(
...
static int __fastcall Script_SetUnitDisplayID(void* L) {
    return SetUnitFieldImpl(L, Game::UNIT_FIELD_DISPLAYID, "Usage: SetUnitDisplayID(unitToken [, displayID])");
}

static int __fastcall Script_RemapDisplayID(void* L) {
    return RemapFieldImpl(L, Game::UNIT_FIELD_DISPLAYID, "Usage: RemapDisplayID(oldDisplayID(s) [, newDisplayID])");
}

static int __fastcall Script_SetUnitMountDisplayID(void* L) {
    return SetUnitFieldImpl(
        L, Game::UNIT_FIELD_MOUNTDISPLAYID, "Usage: SetUnitMountDisplayID(unitToken [, mountDisplayID])"
    );
}

static int __fastcall Script_RemapMountDisplayID(void* L) {
    if (!Game::Lua::IsNumber(L, 1) && !Game::Lua::IsTable(L, 1)) {
        Game::Lua::Error(L, "Usage: RemapMountDisplayID(oldDisplayID(s) [, factionIndexedDisplayIDs])");
        return 0;
    }
    ...
    Game::ClntObjMgrEnumVisibleObjects(RefreshFieldCallback, reinterpret_cast<void*>(Game::UNIT_FIELD_MOUNTDISPLAYID));
    return 0;
}
...
static int __fastcall Script_RemapVisibleItemID(void* L) {
    if (!(Game::Lua::IsNumber(L, 1) || Game::Lua::IsTable(L, 1)) || !Game::Lua::IsNumber(L, 2)) {
        Game::Lua::Error(L, "Usage: RemapVisibleItemID(oldItemID(s), inventorySlot [, newItemID])");
        return 0;
    }
...
static int __fastcall Script_UnitDisplayInfo(void* L) {
    ...
    auto* unit =
        reinterpret_cast<Game::CGUnit_C*>(Game::ClntObjMgrObjectPtr(Game::TYPE_MASK::TYPEMASK_UNIT, nullptr, guid, 0));
    if (unit == nullptr) {
        Game::Lua::Error(L, "Unit not found.");
        return 0;
    }
...
static int __fastcall Script_GetItemDisplayID(void* L) {
    ...
    Game::ItemStats_C* item = Game::DBCache_ItemStats_C_GetRecord(
        Game::g_itemDBCache, static_cast<uint32_t>(Game::Lua::ToNumber(L, 1)), nullptr, nullptr, nullptr, false
    );
    if (item == nullptr) {
...
void RegisterLuaFunctions() {
    Game::FrameScript_RegisterFunction("SetUnitDisplayID", reinterpret_cast<uintptr_t>(&Script_SetUnitDisplayID));
    Game::FrameScript_RegisterFunction("RemapDisplayID", reinterpret_cast<uintptr_t>(&Script_RemapDisplayID));
    Game::FrameScript_RegisterFunction(
        "SetUnitMountDisplayID", reinterpret_cast<uintptr_t>(&Script_SetUnitMountDisplayID)
    );
    Game::FrameScript_RegisterFunction("RemapMountDisplayID", reinterpret_cast<uintptr_t>(&Script_RemapMountDisplayID));
    Game::FrameScript_RegisterFunction(
        "SetUnitVisibleItemID", reinterpret_cast<uintptr_t>(&Script_SetUnitVisibleItemID)
    );
    Game::FrameScript_RegisterFunction("RemapVisibleItemID", reinterpret_cast<uintptr_t>(&Script_RemapVisibleItemID));
    Game::FrameScript_RegisterFunction("UnitDisplayInfo", reinterpret_cast<uintptr_t>(&Script_UnitDisplayInfo));
    Game::FrameScript_RegisterFunction("GetItemDisplayID", reinterpret_cast<uintptr_t>(&Script_GetItemDisplayID));
}
