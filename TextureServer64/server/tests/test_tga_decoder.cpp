...
static auto
make_tga_header(uint16_t w, uint16_t h, uint8_t bpp, uint8_t image_type = 2, uint8_t descriptor = 0) -> std::vector<uint8_t> {
...
static void test_decode_uncompressed_tga() {
    ...
    TexServer::TgaDecoder const dec;
    DecodedTexture tex{};
    bool const ok = TexServer::TgaDecoder::Decode(buf.data(), buf.size(), tex);
    ...
    TEST_ASSERT(p[0] == 0xA0 && p[1] == 0xB0 && p[2] == 0xC0 && p[3] == 0xD0, "row0 pixel0");
    // Row 0, pixel 1 = px_1_0
    TEST_ASSERT(p[4] == 0xE0 && p[5] == 0xF0 && p[6] == 0x01 && p[7] == 0x02, "row0 pixel1");
    // Row 1, pixel 0 = px_0_1
    TEST_ASSERT(p[8] == 0x10 && p[9] == 0x20 && p[10] == 0x30 && p[11] == 0x40, "row1 pixel0");
    // Row 1, pixel 1 = px_1_1
    TEST_ASSERT(p[12] == 0x50 && p[13] == 0x60 && p[14] == 0x70 && p[15] == 0x80, "row1 pixel1");
...
static void test_decode_24bit_tga() {
    ...
    TEST_ASSERT(p[0] == 0x11 && p[1] == 0x22 && p[2] == 0x33 && p[3] == 0xFF, "row0 pixel0 alpha");
    // pixel(1,0): BGR 0x44,0x55,0x66 -> BGRA 0x44,0x55,0x66,0xFF
    TEST_ASSERT(p[4] == 0x44 && p[5] == 0x55 && p[6] == 0x66 && p[7] == 0xFF, "row0 pixel1 alpha");
    // pixel(0,1): BGR 0x77,0x88,0x99 -> BGRA 0x77,0x88,0x99,0xFF
    TEST_ASSERT(p[8] == 0x77 && p[9] == 0x88 && p[10] == 0x99 && p[11] == 0xFF, "row1 pixel0 alpha");
    // pixel(1,1): BGR 0xAA,0xBB,0xCC -> BGRA 0xAA,0xBB,0xCC,0xFF
    TEST_ASSERT(p[12] == 0xAA && p[13] == 0xBB && p[14] == 0xCC && p[15] == 0xFF, "row1 pixel1 alpha");
