...
static void test_slot_lifecycle() {
    ...
    // Allocate a slot.
    int32_t const slot = shm.AllocateSlot();
...
static void test_slot_exhaustion_and_reclaim() {
    ...
    int32_t const overflow = shm.AllocateSlot();
    assert(overflow == -1);

    // Free one slot and verify we can reclaim it.
    int32_t const freed_slot = slots[32]; // pick one in the middle
    shm.FreeSlot(freed_slot);

    int32_t const reclaimed = shm.AllocateSlot();
