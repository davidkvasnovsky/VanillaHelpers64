...
// ---------------------------------------------------------------------------
static auto BuildSyntheticBlp(uint32_t width, uint32_t height, uint32_t alpha_depth) -> std::vector<uint8_t> {
    uint32_t const pixel_count = width * height;

    // Calculate alpha data size
    size_t alpha_size = 0;
    if (alpha_depth == 8) {
        alpha_size = pixel_count;
    } else if (alpha_depth == 4) {
        alpha_size = (pixel_count + 1) / 2;
    } else if (alpha_depth == 1) {
        alpha_size = (pixel_count + 7) / 8;
    }

    size_t const mip0_size = pixel_count + alpha_size;

    // Mip data starts right after header (156) + palette (1024)
    uint32_t const mip0_offset = 156 + (256 * 4);
    size_t const total_size = mip0_offset + mip0_size;
...
// ---------------------------------------------------------------------------
static auto test_parse_blp1_header() -> bool {
    auto buf = BuildSyntheticBlp(64, 64, 0);

    BlpDecoder const decoder;
    BlpInfo info{};
...
// ---------------------------------------------------------------------------
static auto test_reject_invalid_magic() -> bool {
    auto buf = BuildSyntheticBlp(64, 64, 0);
...
// ---------------------------------------------------------------------------
static auto test_decode_palette_texture() -> bool {
...
// ---------------------------------------------------------------------------
static auto test_decode_palette_alpha8() -> bool {
...
// ---------------------------------------------------------------------------
static auto test_decode_palette_alpha1() -> bool {
    auto buf = BuildSyntheticBlp(4, 4, 1);

    // Alpha data is at mip0_offset + pixel_count
    uint32_t const mip0_offset = 156 + (256 * 4);
    uint32_t const pixel_count = 4 * 4;
...
// ---------------------------------------------------------------------------
static auto test_reject_truncated_data() -> bool {
    BlpDecoder const decoder;
    BlpInfo info{};
...
// ---------------------------------------------------------------------------
static auto test_jpeg_stub_returns_false() -> bool {
    auto buf = BuildSyntheticBlp(4, 4, 0);
