...
static inline auto ReadU32(const uint8_t* p) -> uint32_t {
...
auto BlpDecoder::ParseHeader(const uint8_t* data, size_t size, BlpInfo& info) -> bool {
    ...
    // Check magic
    bool const is_blp1 = (data[0] == 'B' && data[1] == 'L' && data[2] == 'P' && data[3] == '1');
    bool const is_blp2 = (data[0] == 'B' && data[1] == 'L' && data[2] == 'P' && data[3] == '2');
    ...
    if (is_blp1) {
        if (size < BLP1_HEADER_SIZE) {
            return false;
        }

        uint32_t const compression = ReadU32(data + 4);
        if (compression > 1) {
        ...
        if (size < BLP2_HEADER_SIZE) {
            return false;
        }

        uint32_t const type = ReadU32(data + 4);
        (void)type; // always 1

        uint8_t const comp = data[8];
        if (comp < 1 || comp > 3) {
...
auto BlpDecoder::Decode(const uint8_t* data, size_t size, DecodedTexture& result) -> bool {
    BlpInfo info{};
    ...
    printf(
        "[BlpDecoder] %s %ux%u comp=%u alpha_depth=%u alpha_type=%u mips=%u\n",
    ...
    switch (info.compression) {
        ...
        case BlpCompression::Uncompressed:
            ...
            {
                uint32_t const mip0_off = info.mip_offsets[0];
                uint32_t const mip0_sz = info.mip_sizes[0];
                if (static_cast<uint64_t>(mip0_off) + mip0_sz > size) {
                    return false;
                }
...
auto BlpDecoder::DecodeJpeg(const uint8_t* data, size_t size, const BlpInfo& info, DecodedTexture& result) -> bool {
    if (info.mip_count == 0) {
        return false;
    }
    ...
    uint32_t const jpeg_hdr_size = ReadU32(data + 156);
    if (jpeg_hdr_size == 0 || jpeg_hdr_size > 1024) {
        printf("[BlpDecoder] JPEG header size=%u (suspicious)\n", jpeg_hdr_size);
        // Still try if <= 4096
        if (jpeg_hdr_size == 0 || jpeg_hdr_size > 4096) {
            return false;
        }
    }

    if (160 + jpeg_hdr_size > size) {
        return false;
    }
    ...
    int channels = 0;
    uint8_t* decoded =
        stbi_load_from_memory(jpeg_buf.data(), static_cast<int>(jpeg_buf.size()), &w, &h, &channels, 4); // request RGBA
    ...
    uint32_t const pixel_count = static_cast<uint32_t>(w) * static_cast<uint32_t>(h);
...
auto BlpDecoder::DecodePalette(const uint8_t* data, size_t size, const BlpInfo& info, DecodedTexture& result) -> bool {
    size_t const palette_offset = info.is_blp2 ? BLP2_HEADER_SIZE : BLP1_PALETTE_OFFSET;
    ...
    uint32_t const mip0_offset = info.mip_offsets[0];
    uint32_t const mip0_size = info.mip_sizes[0];
    ...
    uint64_t const pixel_count_64 = static_cast<uint64_t>(info.width) * info.height;
    if (pixel_count_64 > 8192U * 8192U) {
        return false;
    }
    auto const pixel_count = static_cast<uint32_t>(pixel_count_64);
    ...
    for (uint32_t i = 0; i < pixel_count; ++i) {
        ...
        if (info.alpha_depth == 0) {
            ...
            uint8_t const byte = alpha_data[i / 2];
            if (i % 2 == 0) {
                alpha = (byte & 0x0F) * 17;
            } else {
            ...
            uint8_t const byte = alpha_data[i / 8];
            alpha = (((byte >> (i % 8)) & 1) != 0) ? 0xFF : 0x00;
        }
...
auto BlpDecoder::DecodeDxt(const uint8_t* data, size_t size, const BlpInfo& info, DecodedTexture& result) -> bool {
    if (info.mip_count == 0) {
        return false;
    }
    ...
    for (uint32_t i = 0; i < info.mip_count; ++i) {
        uint32_t const mip_off = info.mip_offsets[i];
        uint32_t const mip_sz = info.mip_sizes[i];
    ...
    if (contiguous && info.mip_count > 0) {
        uint32_t const start = info.mip_offsets[0];
    ...
    switch (info.alpha_type) {
        case 0:
            result.format = TexProto::PixelFormat::DXT1;
            break;
        case 1:
            result.format = TexProto::PixelFormat::DXT3;
            break;
        case 7:
            result.format = TexProto::PixelFormat::DXT5;
            break;
        default:
            result.format = TexProto::PixelFormat::DXT1;
            break;
    ...
    printf(
        "[BlpDecoder] DXT pass-through: %ux%u, alpha_type=%u, mips=%u, %zu bytes\n",
        info.width,
        info.height,
        info.alpha_type,
        info.mip_count,
