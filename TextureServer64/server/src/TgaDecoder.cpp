...
auto TgaDecoder::Decode(const uint8_t* data, size_t size, DecodedTexture& result) -> bool {
    ...
    const uint64_t pixel_data_size = static_cast<uint64_t>(w) * h * src_bytes_per_pixel;
    ...
    for (uint32_t y = 0; y < h; ++y) {
        ...
        uint8_t* row_dst = result.pixels.data() + (static_cast<size_t>(dst_row) * dst_stride);
