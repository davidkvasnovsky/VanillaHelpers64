...
static bool PipeTransaction(const TexProto::Request& req,
                            const char* path,
                            const uint8_t* data,
                            uint32_t data_size,
                            TexProto::Response& resp)
{
    ...
    ok = ok && WriteFile(pipe, &req, sizeof(req), &written, nullptr)
            && (written == sizeof(req));

    // 2) Send path bytes (if any)
    if (ok && req.path_len > 0) {
        ok = WriteFile(pipe, path, req.path_len, &written, nullptr)
          && (written == req.path_len);
    }
    ...
    if (ok && data_size > 0 && data != nullptr) {
        ...
        while (ok && remaining > 0) {
            ...
            ok = WriteFile(pipe, ptr, chunk, &written, nullptr)
              && (written == chunk);
            ptr       += written;
            remaining -= written;
        }
    ...
    if (ok) {
        ...
        while (ok && totalRead < sizeof(resp)) {
            ok = ReadFile(pipe, respBuf + totalRead,
                          sizeof(resp) - totalRead, &bytesRead, nullptr) != FALSE;
            if (ok) totalRead += bytesRead;
        }
...
void ReleaseSlot(int32_t slot) {
    ...
    InterlockedExchange(reinterpret_cast<volatile LONG*>(&hdr->state),
